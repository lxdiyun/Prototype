...
static CreateFoodHeader *CREATE_FOOD_HEADER;
	...
	case FOOD_DESC:
		{
			...
			if (cell == nil) 
			{
				...
				if (nil == FOOD_DESC_TEXT_VIEW)
				{
					...
					//FOOD_DESC_TEXT_VIEW.editable = NO;
...
- (UIView *) tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
	if (FOOD_IMAGE == section)
	{
		...
		{
			CREATE_FOOD_HEADER = [[CreateFoodHeader alloc] initWithFrame:CGRectMake(0.0, 
												0.0, 
...
- (void) textFieldDidBeginEditing:(UITextField *)textField
{
	for (int i = 0; i < NEW_FOOD_DETAIL_MAX; ++i)
	{
		...
		{
			[self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:FOOD_DETAIL] atScrollPosition:UITableViewScrollPositionTop animated:YES];
